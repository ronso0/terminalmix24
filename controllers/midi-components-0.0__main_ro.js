    ...
    **/
    ...
    var EffectUnit = function(unitNumbers, allowFocusWhenParametersHidden, colors) {
            ...
            if (this.hasInitialized === true) {
                for (var n = 1; n <= 3; n++) {
                    var effect = "[EffectRack1_EffectUnit" + this.currentUnitNumber +
                                "_Effect" + n + "]";
                    // ronso0
                    // Don't enable meta knob softTaker in the first place.
        ...
        this.EffectUnitKnob.prototype = new Pot({
            group: this.group,
            /* ronso0
            Disabled because when shift is pressed (i.e. for brake)
            simultaneous use of Fx Meta should be possible.
            (shifted it would scroll WEffectSelector)

            Do the same for shift and unshift */
            function() {
                this.input = function(channel, control, value, _status, _group) {
                    ...
                    if (this.previousValueReceived === undefined) {
                        var effect = "[EffectRack1_EffectUnit" + eu.currentUnitNumber +
                                    "_Effect" + this.number + "]";
                        // ronso0
                        // Disable meta knob softTakeover for direct response
                        // Also change it in
                        // this.setCurrentUnit > this.hasInitialized
                        engine.softTakeover(effect, 'meta', false);
                        engine.softTakeover(effect, "parameter1", false);
                        engine.softTakeover(effect, "parameter2", false);
                        engine.softTakeover(effect, "parameter3", false);
